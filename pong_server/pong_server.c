...
/*** TO BE DONE START ***/
	...
	if(clock_gettime(CLOCK_TYPE, &time2) == -1)
		fail_errno("Could not set clock in time2\n");
...
/*** TO BE DONE START ***/
	...
	if(clock_gettime(CLOCK_TYPE, &time3) == -1)
		fail_errno("Could not set clock in time3\n");
...
/*** TO BE DONE START ***/
	...
	if((gai_rv = getaddrinfo(NULL, port_number_as_str, &gai_hints, &pong_addrinfo)) < 0)
		fail_errno("Could not set port number\n");
	
	if((bind_rv = bind(udp_socket, pong_addrinfo->ai_addr, pong_addrinfo->ai_addrlen)) == 0){
		
		//Salvo il numero di porta libero trovato al valore puntato da pong_port...
		*pong_port = port_number;
		//e ritorno il fd del socket creato
		return udp_socket;
	}
...
/*** TO BE DONE START ***/
	...
	//Se request_socket ha come errno == EAGAIN o EWOULDBLOCK allora significa che non ci sono problemi
	//con la accept(): semplicemente non è arrivata una nuova connessione, e si continua
	if(request_socket >= 0){
	...
	else if(errno == EINTR) continue;		//Quando la accept viene interrotta, e voglio ritentare senza fallire
	else{
		perror("Request socket has not created correctly\n");
		close(server_socket);
		exit(EXIT_FAILURE);
	}
...
/*** TO BE DONE START ***/
	...
	//Recupero le informazioni sull'indirizzo
	if((gai_rv = getaddrinfo(NULL, argv[1], &gai_hints, &server_addrinfo)) < 0)
		fail_errno("Could not get address info");
	
	//Creo il socket
	if((server_socket = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP)) < 0){
		printf("Socket Creation Error in main\n");
		return -1;
	}
	
	//Bindo il socket ad una porta
	if(bind(server_socket, server_addrinfo->ai_addr, server_addrinfo->ai_addrlen) < 0){
		printf("\nPort already in use\n");
		return -1;
	}
	
	//Metto il server in modalità passiva ed attendo una connessione
	if(listen(server_socket, LISTENBACKLOG) < 0){
		printf("\nListen Error in main\n");
	}
