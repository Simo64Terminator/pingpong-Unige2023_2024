...
/*** TO BE DONE START ***/
	...
	if(sprintf(message, "%d", msg_no) < 0)
		fail_errno("Could not copy mgs_no in message\n");
...
/*** TO BE DONE START ***/
	...
	if(clock_gettime(CLOCK_TYPE, &send_time) == -1)
		fail_errno("Could not set time in send_time\n");
...
/*** TO BE DONE START ***/
	...
	if(clock_gettime(CLOCK_TYPE, &recv_time) == -1)
		fail_errno("Could not set time in recv_time\n");
...
/*** TO BE DONE START ***/
  	...
  	int s = getaddrinfo(argv[1], argv[2], &gai_hints, &server_addrinfo);
...
/*** TO BE DONE START ***/
	...
	//Comportamento di default in modalità bloccante
