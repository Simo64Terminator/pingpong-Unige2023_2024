...
/*** TO BE DONE START ***/
	...
	if(sprintf(message, "%d", msg_no) < 0)
		fail_errno("Could not copy msg_no in message\n");
...
/*** TO BE DONE START ***/
	...
	if(clock_gettime(CLOCK_TYPE, &send_time) == -1)
		fail_errno("clock_gettime() send_time error\n");
...
/*** TO BE DONE START ***/
	...
	if (sent_bytes == -1)
		fail_errno("Could not send message\n");

	if (sent_bytes != msg_size)
	{
		debug(" Mmmh...Could not send all bytes. Only %zd bytes have been sent\n", sent_bytes);
	}
...
/*** TO BE DONE START ***/
	...
	if(clock_gettime(CLOCK_TYPE, &recv_time) == -1)
		fail_errno("clock_gettime() recv_time error\n");
...
/*** TO BE DONE START ***/
	...
	//Salvo i flag precedenti dell'fd di ping_socket...
	int flags = fcntl(ping_socket, F_GETFL);
	
	//E li reinserisco insieme al flag O_NONBLOCK (discorso su maschera di bit, l'OR si occupa di inserirli)
	if(fcntl(ping_socket, F_SETFL, O_NONBLOCK | flags) < 0)
		fail_errno("Could not set flags for socket");
...
/*** TO BE DONE START ***/
	...
	if((ask_socket = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP)) < 0)
		fail_errno("TCP Socket Creation Failed\n");

	if(connect(ask_socket, server_addrinfo->ai_addr, server_addrinfo->ai_addrlen) < 0)
		fail_errno("TCP Socket Connection Failed");
...
/** TO BE DONE START ***/
	...
	size_t msg_length = strlen(request);
	
	if (write(ask_socket, request, msg_length) != msg_length)
		fail_errno("Writing Request Error");
...
/*** TO BE DONE START ***/
	...
	if(strncmp(answer, "OK", 2)){
		printf("... Pong server did not agree :c\n");
